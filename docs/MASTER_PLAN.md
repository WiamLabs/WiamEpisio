\#  
████████████████████████████████████████████████████████████  
\# WIAMAPP — MASTER PLAN V5  
\# The Complete Strict End-to-End Blueprint — Global Multi-Country Architecture  
\# © 2026 WiamApp. Powered by WiamLabs  
\# Founder: Martin | wiamapp.com  
\# Version: 5.0 — Updated: July 2026 — V4 FINAL body preserved intact, see SECTION 29 for what changed  
\# ████████████████████████████████████████████████████████████
...
═══════════════════════════════════════════════════════════════  
SECTION 26 — RENDER DEPLOYMENT (UPDATED JULY 2026 — MONOREPO)  
═══════════════════════════════════════════════════════════════

SUPERSEDED: the old instructions below this line described fixing
3 separately-pushed repos. As of July 2026, WiamApp is ONE GitHub
repo (github.com/WiamLabs/WiamApp) with THREE Render services
defined in a single render.yaml Blueprint at the repo root:

wiamapp-backend         — rootDir: backend         — Express API
wiamapp-marketing       — rootDir: marketing       — public site, wiamapp.com
wiamapp-business-web    — rootDir: business-web    — NO public domain, internal only

TO DEPLOY FROM SCRATCH:  
1\. Push the monorepo to github.com/WiamLabs/WiamApp (main branch)  
2\. Render dashboard → New + → Blueprint → connect that repo  
3\. Render reads render.yaml and proposes all 3 services at once  
4\. Fill in every env var marked sync: false in render.yaml, per service  
5\. Map wiamapp.com → wiamapp-marketing only  
6\. Do NOT map a public domain to wiamapp-business-web — it is reached  
   exclusively via wiamapp.com/business/\* through a rewrite proxy  
   defined in marketing/next.config.js (see Section 29\)  
7\. Copy wiamapp-business-web's onrender.com URL into wiamapp-marketing's  
   BUSINESS\_ORIGIN env var — this is the one manual link between the two

CRITICAL: Do not add environment variables to GitHub.  
They go ONLY in Render environment settings, per service.  
The .env file on your computer stays only on your computer.

═══════════════════════════════════════════════════════════════  
SECTION 27 — DATABASE MIGRATION RUN ORDER (UPDATED JULY 2026)  
═══════════════════════════════════════════════════════════════
...
017_trust_system.sql
018_enhanced_profiles.sql
019_enterprise_locations.sql
020_recurring_contracts.sql
021_enterprise_vendors.sql
022_sla_contracts.sql
023_enterprise_invoices.sql
024 through 027               ← eligibility, billing, careers-related
028_eligibility_billing_covers.sql
029 through 033                ← growth/enterprise tables
034_careers_page.sql
035_referrals_photos_disputes_trust.sql   ← NEW July 2026, see Section 29
categories_seed.sql          ← ALWAYS LAST
...
You should now see more than 40 tables when complete (referrals,
booking_photos, disputes added in 035).
...
═══════════════════════════════════════════════════════════════  
SECTION 29 — BUILD STATUS UPDATE (JULY 2026)  
═══════════════════════════════════════════════════════════════

This section records what changed after V4 FINAL was written. It
does not replace any section above — it supplements it. Where this
section conflicts with an earlier one, THIS section is correct.

29.1 — REPOSITORY STRUCTURE CHANGED TO MONOREPO

WiamApp is now ONE GitHub repo (github.com/WiamLabs/WiamApp), not
three. Structure:

  backend/        Express API (was WiamAppExpo/backend)
  database/       SQL migrations (was WiamAppExpo/database)
  mobile/         Expo app (was the rest of WiamAppExpo)
  marketing/      Next.js, wiamapp.com (was WiamAppMarketing)
  business-web/   Next.js, business portal (was WiamAppBusiness)
  render.yaml     defines all 3 deployed services from this one repo

One `git push` now redeploys whichever service actually changed.
See Section 26 for the Render Blueprint setup.

29.2 — DOMAIN ARCHITECTURE CONFIRMED, BUG FIXED

Section 1 always specified wiamapp.com/business/* (path-based, no
subdomain) — that was correct from the start. What had actually been
BUILT in the marketing site's code deviated from this: CTA buttons
pointed at a placeholder subdomain (app.wiamapp.com) that was never
built. Fixed in July 2026:

  wiamapp.com/business/login       →  business-web, proxied
  wiamapp.com/business/apply       →  business-web, proxied
  wiamapp.com/business/dashboard/* →  business-web, proxied
  wiamapp.com/business (bare)      →  marketing's own pitch page

business-web sets basePath: '/business' in its own next.config.js so
every route/redirect/static asset it generates already matches these
targets. marketing/next.config.js rewrites() proxies those specific
paths to business-web's Render URL (BUSINESS_ORIGIN env var). No
public domain is ever mapped to business-web directly — a browser
never learns two services are involved.

api.wiamapp.com remains the one legitimate subdomain — machine-facing
only (mobile app + both web apps call it), never seen by a human.

29.3 — NEW SYSTEMS BUILT (migration 035)

- REFERRALS: every user gets an auto-generated code (users.referral_code).
  Reward fires when a referred worker gets verified (1 free month Pro)
  or a referred customer completes their first booking (cash credit).
  Endpoints: GET/POST /api/referrals/me, /apply. Screen: ReferralScreen.js.

- BEFORE/AFTER PHOTOS: booking_photos table, tied to a booking, phase
  before|after. Protects against disputes, doubles as portfolio content.
  Endpoints: POST/GET /api/bookings/:id/photos. Screen: BookingPhotosScreen.js.

- DISPUTE RESOLUTION: disputes table with evidence_urls, status workflow
  (open → under_review → resolved_customer/worker/split/dismissed).
  Filing one flips the booking to 'disputed' via trigger and freezes
  payout. Endpoints: backend/routes/disputes.js (file, evidence upload,
  admin resolve). Screen: DisputeScreen.js.

- CUSTOMER TRUST SCORE: users.customer_trust_score (0-100), computed
  from completed/cancelled/disputed booking counts via
  recalculate_customer_trust_score(). Visible to a worker on an
  incoming job request BEFORE they accept — closes the "trust is only
  one-directional" gap flagged in worker feedback.

- WORKER PROFILE COMPLETION CHECKLIST: worker_profiles.profile_completion_percent,
  recalculated automatically by DB triggers whenever bio, rate,
  verification status, portfolio, or category changes — no manual
  wiring needed per route. Shown on WorkerDashboardScreen until 100%.

- SCHEDULED BOOKING CONFIRMATION: bookings.worker_confirmed_slot +
  is_future_booking. Worker formally confirms a future date via
  PATCH /api/bookings/:id/schedule-confirm; customer gets notified.

- TWO NEW CRON JOBS: repeat-booking-reminders (nudges a customer to
  rebook a worker they used 6-8 weeks ago and never returned to) and
  scheduled-booking-reminders (24h-ahead nudge for confirmed future
  bookings). Add both to cron-job.org alongside the existing jobs.

29.4 — WEB REGISTRATION BUILT (was a placeholder, now real)

wiamapp.com/register handles both customer and worker signup, with
OTP verification, calling the real /api/auth/register endpoint.
Workers additionally get, right on the web, before ever touching the
app: a profile-completion step (bio, hourly rate) and a full ID +
selfie upload step feeding the SAME admin review queue the mobile app
uses. A confirmation email fires once verification is submitted
(sendVerificationSubmittedEmail, backend/lib/resend.js), so a worker
who registered on the web is never left wondering what happens next.

29.5 — A REAL BUG FIXED

Worker verification approval (backend/routes/verification.js) was
only updating users.is_verified — never worker_profiles.is_verified,
which is what the booking route actually checks before allowing
anyone to book a worker. Every approved worker would have stayed
silently unbookable. Fixed: approval now updates both.

29.6 — DELIBERATELY NOT BUILT

An admin panel for the 15 actions in backend/routes/admin.js (approve
verification, resolve disputes, suspend accounts, view stats) does
NOT exist anywhere — not on web, not in the app. This is intentional:
WiamLabs is building a separate internal Studio covering all WiamLabs
products (not just WiamApp), and admin.js's endpoints are left ready
and untouched for that Studio to call. Do not build a WiamApp-only
admin panel — check with the Studio project first.

29.7 — SMALLER GAPS CLOSED

- /forgot-password → /reset-password: shared password reset flow for
  ANY role (customer, worker, business), lives on marketing since the
  flow has nothing business-specific about it.
- sitemap.xml + robots.txt added to marketing (business dashboard
  correctly excluded from indexing).
- mobile/.env.example was missing entirely (code referenced it, file
  didn't exist) — created with all 11 real EXPO_PUBLIC_ vars, with
  EXPO_PUBLIC_SMILE_IDENTITY_* flagged as present-but-dormant (no
  screen calls it yet — verification today is 100% manual admin
  review of uploaded photos, not automated government ID lookup).

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  
Version: 5.0 — Updated July 2026 — V4 FINAL + Section 29 addendum  
