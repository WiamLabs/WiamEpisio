...
STAGE 2 — Scene-aware sampling
  ...
  → This feeds accurate samples into Stages 3–4 instead of guessing
        ↓
STAGE 3 — Raw footage quality (runs in background, ~2–5 min)
  → OpenCV blur score across sampled frames → banded:
       Sharp = Excellent/Good → auto-clear
       Soft/blurry = Borderline → human review
       Consistently out-of-focus = Poor → auto-reject
  ...
  → FFmpeg: black-frame / frozen-frame detection
        ↓
STAGE 3B — Transcode integrity (after WiamEpisio compresses for delivery)
  → VMAF compares creator's original file vs. our transcoded output
  → Confirms our own pipeline didn't introduce new quality loss
  → This runs on every episode as a system self-check, separate from
    judging the creator's original footage (see Stage 3 above)
        ↓
...
- A creator whose footage lands in the "Good" blur/exposure band publishes without ever waiting on a human — that's still meaningfully better than "any video passes," but it doesn't block a decent, honest upload over an arbitrary line.
- Only genuinely poor output (consistently out-of-focus, real clipping, real dead air) gets auto-rejected — and always with the specific reason, matching your "Changes Required, not just Rejected" rule.
...
review_jobs
  id, episode_id, stage: technical | scene_sampling | raw_footage |
                          transcode_integrity | audio | content_integrity
  status: queued | running | passed | failed | borderline
  started_at, completed_at

review_checks
  id, review_job_id, check_type
    (resolution, aspect_ratio, duration, codec, watermark,       -- technical
     blur_score, brightness, stability, black_frames,            -- raw_footage
     vmaf_score,                                                 -- transcode_integrity only
     loudness, dialogue_presence, silence_gaps,                  -- audio
     duplicate_match)                                            -- content_integrity
