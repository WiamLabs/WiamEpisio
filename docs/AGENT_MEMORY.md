...
**Last updated:** 2026-07-20 — **Founder `/founder/` 500 fix:** EPISIO_SLIM skips wiambot/classics blueprints but founder nav used `url_for('wiambot…')` / `url_for('classics…')` → Jinja BuildError. Fix: `safe_url_for` context processor + templates; overview queries hardened; Episio Season QC nav link. Founder accounts are intentionally locked to dashboard routes (`enforce_team_account_isolation`) — that is why browsing the public site “sticks” on founder. Deploy this fix to Render.

**Previous — Render LIVE on episio.wiamlabs.com** (review-status 401, founder QC 302). Log warnings only (pip root, urllib3/chardet, seed context fixed, web_book_content skip OK on fresh Episio DB). Built: invite generate/redeem → Studio direct + full public profile editor; Register eye + username check + last name. HTML: 111 mockups / 101 screens — 28 thin shells remain. QC plan in Downloads still says “plan only” but toolkit+SLA already shipped; episode band stays **4–5 min** (Martin law) not plan’s 3–6.
