...
**Last updated:** 2026-07-20 — **Host law (Martin):** Episio API = `https://episio.wiamlabs.com`. Video CDN/R2 = `https://video.episio.wiamlabs.com`. **Never use `wiamapp.com` for Episio** (separate workers app). Render auto-deploy did NOT run — Martin must **Manual Deploy** Flask service from `episio-flask-deploy` @ `1e54439`. Live `episio.wiamlabs.com` still old build (media-specs 200, but `review-status` / founder QC 404). Ship fixes already on GitHub: restored `server.py` + lean `requirements.txt`.
