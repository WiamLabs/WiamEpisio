...
**Last updated:** 2026-07-20 — **Pushed to GitHub/Render:** `master` + `episio-full-ship` @ `392b6e6` (clean orphan deploy — GitHub blocked full history due to old `WiamApp.env` Cohere/Groq keys). Excluded from push: `_local_secrets_hold/`, `.env.render`, WiamTrade*, Luma ad prompts, bank PDF. Render should auto-deploy from `master`. Local full history still recoverable from reflog if needed.
