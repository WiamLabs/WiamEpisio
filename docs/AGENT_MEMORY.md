...
**Last updated:** 2026-07-20 — **Render deploy diagnosis:** Live `wiamapp.com` is the **Next.js workers marketplace** (`main` branch) — NOT Flask Episio. Flask lives at `https://wiamapp.onrender.com` (health OK). First orphan ship `3c293bb` was missing `server.py` → builds failed / old instance stayed. Fixed in `6be6065` (+ bot/core/…). Also stripped opencv from `requirements.txt` → `requirements-qc.txt` so Render free image can boot. Push branches: `master` / `episio-flask-deploy` / `episio-full-ship`. If auto-deploy off: **Manual Deploy** that Flask service from `episio-flask-deploy`. Do NOT point `wiamapp.com` at workers `main` if Episio API is expected there.
