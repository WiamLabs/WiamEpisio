...
const ProfileScreen = ({ navigation }) => {
  ...
  const roleBadge = isFounder
    ...
    : { label: 'Reader', bg: 'rgba(52,152,219,0.15)', color: '#5dade2' };
    ...
    return (
      <View style={s.pad}>
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={s.qlRow}>
          ...
          <QuickLink icon={Download} label="Offline" color="#60a5fa" onPress={() => navigation.navigate('OfflineReading')} />
          <QuickLink icon={Trophy} label="Stats" color="#f59e0b" onPress={() => navigation.navigate('ReaderStats')} />
          <QuickLink icon={Flame} label="Streaks" color="#ef4444" onPress={() => navigation.navigate('ReadingStreaks')} />
        ...
        {(isCreator || isFounder) ? (
          <>
            ...
            <View style={s.secHead}><LayoutGrid size={16} color={COLORS.secondary} /><Text style={s.secTitle}>Reading</Text></View>
        ...
        <FirstMissionCard
          ...
          onClaimed={async () => {
            try {
              const balRes = await coinsApi.getBalance();
        ...
        {readingBooks.length > 0 && (
          <View style={s.card}>
            <View style={s.cardHead}><BookOpen size={14} color={COLORS.secondary} /><Text style={s.cardTitle}>Continue Reading</Text></View>
            <FlatList
              horizontal showsHorizontalScrollIndicator={false}
              data={readingBooks.slice(0, 6)}
              keyExtractor={i => `cr-${i.id}`}
              renderItem={({ item }) => (
                <TouchableOpacity style={s.libBook} onPress={() => navigation.navigate('BookDetail', { bookId: item.id })}>
          ...
          <View style={s.card}>
            ...
            {creatorStories.slice(0, 5).map((st) => (
              <TouchableOpacity
                ...
                onPress={() => navigation.getParent()?.navigate('Studio', { screen: 'StoryManager', params: { bookId: st.id } })}
              >
    ...
    return (
      <View style={s.pad}>
        ...
        <View style={s.card}>
          ...
          <View style={s.toggleRow}>
            <View style={{ flex: 1 }}><Text style={s.toggleTitle}>Reading Activity</Text><Text style={s.toggleDesc}>Show what you're reading</Text></View>
  ...
  const renderSupport = () => (
    <View style={s.pad}>
      ...
      <View style={s.card}>
        ...
        <SupportLink icon={Bot} title="WiamBot" sub="Chat with our AI assistant" color="#818cf8" onPress={() => navigation.navigate('WiamBot')} />
