...
const CustomDrawerContent = (props) => {
  ...
  const roleBadge = !isAuthenticated
    ? { label: 'Guest', bg: 'rgba(255,255,255,0.06)', color: COLORS.textMuted }
    : isCreator
    ? { label: 'Creator', bg: 'rgba(212,168,67,0.15)', color: COLORS.secondary }
    : { label: 'Watcher', bg: 'rgba(255,255,255,0.06)', color: COLORS.textMuted };
      ...
      <DrawerContentScrollView {...props} contentContainerStyle={styles.scrollContent}>
        ...
        <View style={styles.header}>
          <View style={styles.avatarWrapper}>
            {isAuthenticated && resolveUrl(user?.avatar_url) ? (
              <Image source={{ uri: resolveUrl(user.avatar_url) }} style={styles.avatar} />
            ) : (
              <LetterAvatar name={user?.display_name || 'G'} size={46} letters={2} borderWidth={0} />
            )}
          </View>
          <View style={styles.headerInfo}>
            <Text style={styles.name} numberOfLines={1}>
              {isAuthenticated ? (user?.display_name || 'Watcher') : 'Guest'}
            </Text>
            <Text style={styles.email} numberOfLines={1}>
              {isAuthenticated ? (user?.email || '') : 'Watch free · sign up for coins'}
            </Text>
        ...
        <View style={styles.section}>
          ...
          <NavItem icon={Clapperboard} label="Watch" active={currentRoute === 'MainTabs'} onPress={() => props.navigation.navigate('MainTabs', { screen: 'Home' })} />
          <NavItem icon={Library} label="My Library" onPress={() => { props.navigation.navigate('MainTabs', { screen: 'Library' }); }} />
          {isAuthenticated ? (
            <>
