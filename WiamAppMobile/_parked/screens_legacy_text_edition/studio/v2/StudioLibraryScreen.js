...
const StudioLibraryScreen = ({ navigation }) => {
  ...
  const load = useCallback(async () => {
    try {
      ...
      setSeries(seriesRes.status === 'fulfilled' ? (seriesRes.value?.story_bundles || seriesRes.value?.story_bundle || seriesRes.value?.series || []) : []);
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
          ...
          {series.length === 0 ? (
            ...
            series.map((s) => (
              <EntityRow
                ...
                cover={s.cover_url}
                onPress={() => navigation.navigate('SeriesEditor', { mode: 'edit', seriesId: s.id, storyBundleId: s.id })}
              />
            ))
          )}
        </Section>
          ...
          {universes.length === 0 ? (
            ...
            universes.map((u) => (
              <EntityRow
                key={`universe-${u.id}`}
                title={u.title}
                subtitle={`${u.story_bundle_count || u.series_count || 0} bundles · ${u.visibility}`}
                cover={u.cover_url}
