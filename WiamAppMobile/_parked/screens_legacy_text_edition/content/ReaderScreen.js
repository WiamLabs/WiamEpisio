...
const EMOJIS = ['Γ¥ñ∩╕Å', '≡ƒÿé', '≡ƒÿ¡', '≡ƒÿí', '≡ƒÿ«', '≡ƒöÑ'];
...
const ReaderScreen = ({ route, navigation }) => {
  ...
  // Push 10 ΓÇö next-in-series CTA on the last chapter
  const [nextInSeries, setNextInSeries] = useState(null);

  // Reading progress ΓÇö paragraph-level tracking
  ...
  // ΓöÇΓöÇ Push 3 ΓÇö time-based view counter ΓöÇΓöÇ
  ...
  // ΓöÇΓöÇ Restore reading progress: check AsyncStorage first, then backend ΓöÇΓöÇ
  useEffect(() => {
  ...
  // ΓöÇΓöÇ Scroll to resume paragraph after paragraphs render ΓöÇΓöÇ
  useEffect(() => {
  ...
  // ΓöÇΓöÇ Save reading position every 3 seconds to AsyncStorage + backend ΓöÇΓöÇ
  useEffect(() => {
  ...
  const handleReact = async (paraIdx, emoji) => {
    selectionTick();
    // Optimistic update ΓÇö show reaction immediately
        ...
        text: 'Delete', style: 'destructive', onPress: async () => {
          try {
            ...
            if (topLevelId && topLevelId !== commentId) {
              // Deleting a reply ΓÇö remove it from the parent's replies
              setThreadComments((prev) => prev.map((c) => {
                if (c.id === topLevelId) {
              ...
              // Deleting a top-level comment ΓÇö remove it AND all its replies
  ...
  // Push 10 ΓÇö when reader hits the final chapter, fetch the next book in series.
  useEffect(() => {
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.bg }]}>
      ...
      {/* ΓöÇΓöÇ Top bar: only "CHAPTER X" ΓöÇΓöÇ */}
      ...
      {/* ΓöÇΓöÇ Settings dropdown ΓÇö full panel ΓöÇΓöÇ */}
      {showSettings && (
      ...
      {/* ΓöÇΓöÇ Fixed watermark overlay (does NOT scroll) ΓöÇΓöÇ */}
      ...
      {/* ΓöÇΓöÇ Main scroll content ΓöÇΓöÇ */}
      <ScrollView 
        ref={scrollRef}
        style={styles.scroll}
        ...
        selectable={false}
      >
        {/* Author's Note ΓÇö only chapter 1 and if introduction exists */}
        ...
        {/* Chapter title ΓÇö below Author's Note */}
        {chapter?.title ? (
        ...
        {lockedInfo ? (
          <View style={[styles.lockOverlay, { backgroundColor: colors.panel, borderColor: colors.border }]}>
            ...
            {!lockedInfo.premiumLocked && rewardedReady && rewardedRemaining > 0 && (
              ...
              >
                <Text style={[styles.rewardedBtnText, { color: colors.accent }]}>
                  Γû╢ Watch Ad to Unlock Free ({rewardedRemaining}/day)
                </Text>
              </TouchableOpacity>
            )}
            ...
            <TouchableOpacity style={[styles.walletLink, { marginTop: 4 }]} onPress={() => navigation.navigate('PremiumScreen')}>
              <Text style={[styles.walletLinkText, { color: '#c084fc' }]}>Remove Ads ΓÇö Try Premium</Text>
            </TouchableOpacity>
        </View>
        ) : (
          <View>
            ...
            {paragraphs.map((para, idx) => {
              ...
              return (
                ...
                >
                  ...
                  {/* Paragraph controls ΓÇö emoji btn | reaction badges | comment btn */}
          ...
          >
                  ...
                  {/* Emoji picker ΓÇö appears below controls when active */}
                  {emojiPickerIdx === idx && (
            ...
            {/* ΓöÇΓöÇ Tip Creator Panel ΓöÇΓöÇ */}
            {!isOwnBook && (
          ...
          >
            ...
            {/* ΓöÇΓöÇ Follow / Ad Panel (dynamic transition) ΓöÇΓöÇ */}
            {!isOwnBook && creatorName && (
              <>
                ...
                {!isFollowing && !followDismissed ? (
                  ...
                  /* Once followed (or already following) ΓåÆ show Ad (non-premium only) */
                  !isPremium && (
        ...
        {/* ΓöÇΓöÇ End-of-chapter navigation zone ΓöÇΓöÇ */}
        {!lockedInfo && (
          <View style={[styles.endZone, { borderTopColor: colors.border }]}>
            ...
            {canNext ? (
              <>
                {/* Next Chapter Button ΓÇö user must press to continue */}
                {loadingNext ? (
              ...
              <View style={styles.endStoryWrap}>
                ...
                {/* Push 10 ΓÇö Next book in series */}
                {nextInSeries ? (
                  <TouchableOpacity
                  ...
                  >
                    ...
                    <View style={[styles.nextBookCta, { backgroundColor: colors.accent }]}>
                      <Text style={styles.nextBookCtaText}>Continue the journey ΓåÆ</Text>
                    </View>
                  </TouchableOpacity>
                ) : null}
            </View>
      ...
      {/* ΓöÇΓöÇ Comment Thread Modal ΓöÇΓöÇ */}
      <Modal visible={showThread} transparent animationType="slide" onRequestClose={() => setShowThread(false)}>
        ...
        >
          ...
          {/* ΓöÇΓöÇ Context Menu (inside thread modal) ΓöÇΓöÇ */}
          {contextMenu.visible && (
            <TouchableOpacity
              activeOpacity={1}
